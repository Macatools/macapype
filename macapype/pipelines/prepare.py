...
def create_data_preparation_baboon_pipe(params,
                                        name="data_preparation_baboon_pipe"):
    """
    prepare:
    ...
    if "reorient" in params.keys():
        ...
        if "new_dims" in params["reorient"].keys():
            ...
            reorient_T1_pipe = create_reorient_pipeline(
                                                       name="reorient_T1_pipe",
                                                       new_dims=new_dims)
            data_preparation_baboon_pipe.connect(inputnode,
                                                 ('T1', get_first_elem),
                                                 reorient_T1_pipe,
                                                 'inputnode.image')

            reorient_T2_pipe = create_reorient_pipeline(
                                                       name="reorient_T2_pipe",
                                                       new_dims=new_dims)
            data_preparation_baboon_pipe.connect(inputnode,
                                                 ('T2', get_first_elem),
                                                 reorient_T2_pipe,
    ...
    if "bet_crop" in params.keys():
        ...
        data_preparation_baboon_pipe.connect(denoise_T1, 'output_image',
                                             bet_crop, 't1_file')
        data_preparation_baboon_pipe.connect(denoise_T2, 'output_image',
                                             bet_crop, 't2_file')
    ...
    norm_intensity_T1 = NodeParams(ants.N4BiasFieldCorrection(),
                                   name='norm_intensity_T1')
    norm_intensity_T2 = NodeParams(ants.N4BiasFieldCorrection(),
                                   name='norm_intensity_T2')
    ...
    data_preparation_baboon_pipe.connect(crop_bb_T1, 'roi_file',
                                         norm_intensity_T1, "input_image")
    data_preparation_baboon_pipe.connect(crop_bb_T2, 'roi_file',
                                         norm_intensity_T2, "input_image")
    ...
    data_preparation_baboon_pipe.connect(norm_intensity_T1, 'output_image',
                                         align_T2_on_T1, 'reference')
    data_preparation_baboon_pipe.connect(norm_intensity_T2, 'output_image',
                                         align_T2_on_T1, 'in_file')
